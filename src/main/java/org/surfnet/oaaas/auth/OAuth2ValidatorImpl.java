...
public class OAuth2ValidatorImpl implements OAuth2Validator {
    ...
    protected Client validateClient(AuthorizationRequest authorizationRequest) {
        ...
        if (!client.isAllowedImplicitGrant() && authorizationRequest.getResponseType().equals(IMPLICIT_GRANT_RESPONSE_TYPE)) {
    ...
    protected void validateClient(AccessTokenRequest accessTokenRequest, BasicAuthCredentials clientCredentials) {
        Client client = null;
        ...
        if (!clientCredentials.isNull()) {
            ...
            client = getClient(clientCredentials.getUsername(), clientCredentials.getPassword(), UNAUTHORIZED_CLIENT);
        } else if (!StringUtils.isBlank(accessTokenRequest.getClientId())) {
            // Use the request parameters to obtain the client
            client = getClient(accessTokenRequest.getClientId(), accessTokenRequest.getClientSecret(), UNKNOWN_CLIENT_ID);
        }
